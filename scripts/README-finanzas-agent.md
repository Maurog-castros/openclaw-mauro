...
- **Gmail Santander transferencias** → `data/transferencias.csv`
- **Gmail Santander cartola mensual (PDF)** → `data/santander_cartola.csv` (password PDF en `SANTANDER_CARTOLA_PDF_PASSWORD`)
...
`FINANZAS_OWNER_RUT` mejora descripciones en comprobantes Gmail (ej. «cuenta propia»); todas salen como `transferencia_salida` desde tu Santander.
...
### Cartola Santander + cuadratura

Descarga PDF mensual desde Gmail (`Cartola Mensual de Cuentas`, `mensajeria@santander.cl`):

```powershell
# Password PDF (mismo siempre): secrets/santander_cartola.env.example
python scripts/santander_cartola_agent.py --json
```

Cuadratura mes vs CSV unificado (fecha+monto):

```powershell
python scripts/santander_cuadratura.py --month 2026-04 --json
```

El pipeline (`run_finanzas_pipeline.sh`) incluye cartola + merge automatico.

---

## Alias de comercios (mall chino, lider, etc.)

Archivo: `data/finanzas_merchant_aliases.json` (plantilla en `config/finanzas/merchant_aliases.example.json`)

Cada alias tiene:

- `label`: nombre que usas con el agente (ej. `mall chino`)
- `patterns`: textos que aparecen en `description` o `merchant` del CSV (cartola, boletas)
- `notes`: solo documentacion

Ejemplo: **mall chino** → compras `COMERCIAL KAISHENG` en cartola Santander.

Consulta manual:

```powershell
python scripts/finanzas_merchant_report.py --list
python scripts/finanzas_merchant_report.py --alias "mall chino" --month 2026-04 --json
```

Sin `--month` suma todo el historial del CSV. El agente finanzas debe ejecutar el mismo script (ver `apply_openclaw_finanzas_config.py` / SOUL).

Agregar un comercio: edita el JSON, agrega un objeto en `aliases` con `id`, `label` y `patterns`, guarda y vuelve a preguntar (no hace falta merge).
...
| `scripts/finanzas_merchant_report.py` | Gasto por alias de comercio |
| `scripts/finanzas_monthly_report.py` | Reporte mensual |
| `config/finanzas/merchant_aliases.example.json` | Plantilla alias comercios |
| `data/finanzas_merchant_aliases.json` | Alias activos (editable) |
