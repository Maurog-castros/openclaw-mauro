...
from intel_daily_report import RAW, ROOT, build_report, bullets, clean_title, format_whatsapp, github_topics, refresh_sources, section, top_relevant

DEFAULT_TARGET_FILE = Path("/home/node/.openclaw-secrets/whatsapp_allow_from.txt")


def fmt_item(item: str) -> str:
    item = clean_title(item)
    title = item.split("](")[0].lstrip("[")
    score_match = re.search(r"score:([0-9]+)", item)
    comments_match = re.search(r"comments:([0-9]+)", item)
    metric = ""
    if score_match:
        metric = f" - score {score_match.group(1)}"
        if comments_match:
            metric += f", {comments_match.group(1)} comments"
...
def build_messages(raw: str) -> list[str]:
    ...
    hn_body = "\n".join(fmt_item(item) for item in hn_items[:7]) or "• Sin señales HN relevantes hoy."
    hn_msg = "🔥 *Intel Daily - Hacker News / HN*\n\n" + hn_body

    reddit_body = "\n".join(fmt_item(item) for item in reddit_items[:8]) or "• Sin señales Reddit relevantes hoy."
    reddit_msg = "🧵 *Intel Daily - Reddit*\n\n" + reddit_body

    github_lines = [
        f"• *{repo['name']}* ({repo['stars']} stars): {repo['desc'][:120]}"
        for repo in repos[:8]
    ]
