...
def parse_line_items(text: str) -> List[Tuple[str, Optional[float]]]:
    ...
    for line in lines:
        ...
        if qty_price:
            ...
            # Patrón normal: "... 1590 $" o "... 1.590 $"
            amount_before_currency = re.search(r"([\d\.\,]{2,})\s*\$\s*$", line)
            if amount_before_currency:
                amount = amount_before_currency.group(1).replace(".", "").replace(",", ".")
                try:
        ...
        # Limpieza de prefijos de código de barra al inicio.
